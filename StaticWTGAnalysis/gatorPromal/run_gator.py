...
parser = argparse.ArgumentParser(description='ProMal Gator')
parser.add_argument('--apk_dir', type=str, required=True,
    help='path to directory containing the apks')
parser.add_argument('--adk_dir', type=str, required=True,
    help='path to Android SDK')
args = parser.parse_args()

g_app_dir = args.apk_dir
g_adk_dir = args.adk_dir
g_gator_root = os.path.join(".")
g_process_size = 10
g_timeout = 3600
g_start_index = 0
g_stop_index = 5000
...
def main():
    ...
    for i in range(start_index, stop_index):
        ...
        configs = parse_gator_params([app_path, "-client", "GUIHierarchyPrinterClientHl"])
